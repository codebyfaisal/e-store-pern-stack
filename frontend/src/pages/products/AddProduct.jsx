...
const AddProduct = ({ page }) => {
  const { data, fetchData} = useApiDataStore();
  ...
  const getProductMeta = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${backendApiUrl}/api/products/meta`);
      console.log("Product meta fetched successfully:", response.data);
      const { brands, categories } = response.data?.result;

      if (!brands || !categories) {
        throw new Error("Brands or Categories missing in response.");
      }
      ...
      toast.error(
        "Failed to load form options. Please try again later. or Reload."
      );
    } finally {
      setLoading(false);
    }
