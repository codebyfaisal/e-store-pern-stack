...
const SalesReport = () => {
  const { data, fetchData, loading, error } = useApiDataStore();

  useEffect(() => {
    fetchData("/api/reports/sales");
  }, [fetchData]);
  ...
  const isReportData = data && typeof data === "object" && !Array.isArray(data);
  if (!isReportData || Object.keys(data).length === 0)
    return (
      <NoItemError error="No sales report data available. Try again later." />
    );
